...
@section('content')
    <div class="row justify-content-center">
        <div class="col-lg-9">
            <h3 class="fw-bold mb-4">People</h3>
            ...
            <div>
                <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3" style="border-color: var(--primary-color) !important;">
                    <h5 class="fw-bold text-primary mb-0">Members</h5>
                    <div class="d-flex align-items-center gap-3">
                        <span class="text-muted small">{{ $members->count() }} students</span>
                ...
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden" style="background: var(--card-bg); border: 1px solid var(--border-color) !important;">
                    ...
                    @else
                        <div class="list-group list-group-flush">
                            ...
                            <div class="list-group-item bg-transparent border-0 py-3 px-4 d-flex align-items-center gap-3 hover-bg">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($member->name) }}&background=random" class="rounded-circle shadow-sm" width="40" height="40">
                                <div class="flex-grow-1">
                                    <h6 class="fw-bold mb-0">{{ $member->name }}</h6>
                                    <small class="text-muted">Joined {{ $member->pivot->created_at->format('M d, Y') }}</small>
                                </div>
                                @if(auth()->id() === $classroom->teacher_id)
                                    <form action="{{ route('courses.kick', [$classroom, $member]) }}" method="POST" onsubmit="return confirm('Kick this member?')">
                                        @csrf
